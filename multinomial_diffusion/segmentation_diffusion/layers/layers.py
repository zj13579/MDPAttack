...
class SegmentationUnet(nn.Module):
    def __init__(self, num_classes, dim, num_steps, dataset_name, dim_mults=(1, 2, 4, 8), groups = 8, dropout=0.):
        super().__init__()
        ...
        if dataset_name == 'filmtrust':
            self.target_shape_1 = (32, 64, 32, 64)
            self.target_shape_2 = (32, 9, 33, 61)
        elif dataset_name == 'ml-100k':
            self.target_shape_1 = (32, 64, 32, 64)
            self.target_shape_2 = (32, 6, 33, 51)
    ...
    def forward(self, time, x):
        ...
        x = F.interpolate(x, size=self.target_shape_1[2:], mode='nearest')
        ...
        x = self.final_conv(x)
        x = F.interpolate(x, size=self.target_shape_2[2:], mode='nearest')
        final = x.view(B, self.num_classes, *x_shape)
