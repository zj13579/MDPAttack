...
class GMF(nn.Module):
    def __init__(self, num_user, num_item, result_dirs, mf_dim=10, trainable=True):
        super().__init__()
        ...
        if trainable:
            ...
            trained = torch.load(result_dirs+'/GMF.pt', weights_only=False).state_dict()
...
class MLP(nn.Module):
    def __init__(self, num_user, num_item, result_dirs, mlp_layers=None, trainable=True):
        super().__init__()
        ...
        if trainable:
            ...
            trained = torch.load(result_dirs+'/MLP.pt', weights_only=False).state_dict()
            for name, val in self.named_parameters():
                val.data = trained[name]
                val.requires_grad = False
...
class Neural_MF(nn.Module):
    ...
    def __init__(self, num_user, num_item, result_dirs, mf_dim=10, mlp_layers=None, use_pretrain=True):
        super().__init__()
        if mlp_layers is None:
            mlp_layers = [10]
        self.gmf = GMF(num_user, num_item, result_dirs, mf_dim, trainable=not use_pretrain)
        self.mlp = MLP(num_user, num_item, result_dirs, mlp_layers=mlp_layers, trainable=not use_pretrain)
        self.linear = nn.Sequential(
